...
class TestOFXParser:
    ...
    def test_when_init_then_ofx_parser_returned(self):
    ...
    def test_when_parse_and_banking_transactions_then_return_transations(
        self, isfile, ofx_parser
    ):
        mock_open_obj = mock_open(read_data=self.raw_ofx())
        with patch("builtins.open", mock_open_obj):
            transacions = ofx_parser.parse(
                "banking_transactions", "fake/path/to/file.ofx"
            )
    ...
    def test_when_parse_and_bad_object_type_then_raise_error(self, ofx_parser):
    ...
    def test_when_parse_and_file_path_not_a_file_then_raise_error(
        self, isfile, ofx_parser
    ):
        with raises(OFXParserError) as error_msg:
    ...
    @patch("os.path.isfile", return_value=True)
    def test_when_parse_and_file_not_ofx_or_qfx_then_raise_error(
        self, isfile, ofx_parser
    ):
        with raises(OFXParserError) as error_msg:
